...
def publish_photo_metadata(filename: str) -> bool:
    """
    Publishes a photo metadata payload to the RMQ queue 'presensi'.
    """
    connection = None
    try:
        payload = {
        ...
        logging.info(f"Successfully published photo metadata to RMQ queue '{config.RMQ2_QUEUE}'.")
        return True
    except Exception as e:
        logging.error(f"RMQ PUBLISH FAILED: {e}", exc_info=True)
        return False
